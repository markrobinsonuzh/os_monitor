...
pri_author <- c("altermatt f 0000 0002 4831 6958")
sec_author <- c("altermatt f")
...
dois <- unique(na.omit(m$doi))

# find which dois to remove
system.time(cras <- cr_agency(dois))
agency <- sapply(cras, function(u) {
  v <- u$agency$id[1]
  if(is.null(v)) NA else v
})

# info about DOIs to remove
table(agency, useNA="ifany")
dois_remove <- is.na(agency) | agency!="crossref"
agency[dois_remove]

dois <- setdiff(dois, names(agency)[dois_remove])

oaf <- oadoi_fetch(dois, email="mark.robinson@mls.uzh.ch")
...
# Plot of OA over time: only ZORA records (DOI not required)
...
# Plot of OA over time: ZORA + ORCIDs (articles with DOIs only)
...
# OA percent over time (articles with DOIs only; w/ or w/o blue as OA)
...
# Articles listed as 'closed' overall (articles with DOIs only)
