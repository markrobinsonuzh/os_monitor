...
do
	...
	jq -r '{doi: .doi | ascii_downcase, oa_status: .oa_status, version: .best_oa_location.version, firstversion: .first_oa_location.version} | .doi , .oa_status, .version, .firstversion' /home/data_feed.json | awk '{if(NR%4!=0) printf("%-10s ",$0); else printf("%-10s",$0); if(NR%4==0) printf("\n");}' | awk '{$1=$1;print}' > /home/data_feed.csv
        status3=$?
        if [ $status3 -eq 0 ];
        then
